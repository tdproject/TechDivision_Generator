...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="processor">
		...
		<xsl:variable name="processorName"><xsl:value-of select="$namespace"/>_<xsl:value-of select="$module"/>_Model_Services_<xsl:value-of select="@name"/></xsl:variable>
		<xsl:variable name="className"><xsl:value-of select="$namespace"/>_<xsl:value-of select="$module"/>_Common_Delegates_<xsl:value-of select="@name"/>DelegateRemoteImplementation</xsl:variable>
         ...
         */
         class <xsl:value-of select="$className"/>
			...
			public function __construct(TDProject_Application $application) {
		        ...
		        $connection = TechDivision_ApplicationServerClient_Context_Connection_Factory::createContextConnection();
		        $session = $connection-<xsl:text disable-output-escaping="yes">&gt;</xsl:text>createContextSession();
		        $initialContext = $session-<xsl:text disable-output-escaping="yes">&gt;</xsl:text>createInitialContext();
		        // lookup the remote processor implementation
		        $this-<xsl:text disable-output-escaping="yes">&gt;</xsl:text>_processor = $initialContext-<xsl:text disable-output-escaping="yes">&gt;</xsl:text>lookup(new TechDivision_Lang_String('<xsl:value-of select="$processorName"/>'));
			}
